...
const requestedAbsenceResolvers = {
  Query: {
    getPendingRequestedAbsences: async (
      parent: null,
      args: null,
      context: OurContext
    ) => {
      ...
      const statusId: { dbId: string } | null =
        await context.db.requestStatus.findFirst({
          where: {
  ...
  Mutation: {
    createRequestedAbsence: async (
      parent: null,
      { inputs }: { inputs: RequestedAbsenceCreationInput },
      context: OurContext
    ) => {
      return await context.db.$transaction(async (tx) => {
        ...
        await tx.absenceNotification.update({
          ...
          data: {
            isForBoss: true,
            hasBeenSeen: false,
            message: inputs.isVacation
              ? messages.vacationAbsenceRequest
              : messages.informalAbsenceRequest,
          },
        });
    ...
    makeDecisionRequestedAbsence: async (
      parent: null,
      input: {
        absenceId: string;
      ...
      if (
        (await db.requestedAbsence.count({
          where: {
  ...
  WholeRequestedAbsence: {
    ...
    justification: async (
      ...
      return parent.type !== Enum_Absence_Type.SPONTANEOUS
        ? null
