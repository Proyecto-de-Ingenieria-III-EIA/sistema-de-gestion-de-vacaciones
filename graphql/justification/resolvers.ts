...
import { Enum_Requested_Absence_Status_Name, Enum_Spotaneus_Absence_Status_Name, Justification } from "@prisma/client";
...
const justificationResolvers = {
    Mutation: {
        ...
        updateJustification: async (parent: null, { input }: { input: JustificationCreationInput }, { db }: OurContext) => {
             const existingJustification = await db.justification.findFirst({
                    where: {
                        absenceId: input.absenceId,
                    }
            ...
            if (reqAbsenName.length !== 0)  // If it is a requested absence check the status
            ...
            else {  // Then it has to be spontaneous absence
                // Spontaneous absence
                ...
                if (spontAbsenName.length !== 0 && spontAbsenName[0].name !== Enum_Spotaneus_Absence_Status_Name.PENDING)
                    throw new AbsenceHasBeenResolvedError();
            }
        ...
        addCommentToJustification: async (parent: null, args: { absenceId: string, comments: string }, { db }: OurContext) => {
            if ((await db.justification.count({
                where: {
