...
export function AbsenceGrid({ collaborators, currentDate }: AbsenceGridProps) {
  ...
  const getAbsenceForDay = (
    collaborator: CollaboratorWithAbsences,
    day: number
  ) => {
      ...
      <table className='w-full border-collapse'>
        <thead>
          <tr className='text-center bg-muted/50'>
            <th className='p-3 border-r sticky left-0 bg-muted/50 z-10 min-w-[200px] text-left'>
              Empleado
            </th>
            {days.map((day) => (
              <th
                key={day}
                className='p-2 min-w-[40px] border-r last:border-r-0'
              >
                {day}
              </th>
            ))}
                ...
                return (
                  <td
                    ...
                    className={cn(
                      ...
                      absence ? getAbsenceColor(absence.type) : ''
                    )}
