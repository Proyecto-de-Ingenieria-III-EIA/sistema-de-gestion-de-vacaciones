...
const spontaneousAbsenceResolvers = {
  Mutation: {
    createSpontaneousAbsence: async (
      parent: null,
      ...
      return await context.db.$transaction(async (tx) => {
        ...
        const endDate = inputs.endDate
          ...
          : new Date(startDate.getTime() + 5 * 60 * 1000); // Default to 5 minutes later
    ...
    addEndDateToSpontaneousAbsence: async (
      parent: null,
      ...
      if (absence.createdBy !== context.authData.userId) {
        throw new NotSufficentCredentialsError(
          'You are not allowed to add an end date to this absence'
        );
      }
    ...
    makeDecisionSpontaneousAbsence: async (
      parent: null,
      {
        absenceId,
        decision,
      }: { absenceId: string; decision: Enum_Spotaneus_Absence_Status_Name },
  ...
  CompleteSpontaneousAbsence: {
    colaborator: (
      parent: CompleteSpontaneousAbsence,
      args: null,
      context: OurContext
    ) => {
  ...
  Query: {
    getPendingSpontaneousAbsences: async (
      _parent: null,
      _args: null,
      ctx: OurContext
    ) => {
